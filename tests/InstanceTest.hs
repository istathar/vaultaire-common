...
suite = do
  describe "Round trip through Read and Show instances" $ do
    ...
    it "Reads ISO 8601 timestamps" $ do
      (read "2014-07-31T23:23:35.948797001Z" :: TimeStamp) `shouldBe` (TimeStamp 1406849015948797001)
      (read "2014-07-31T23:23:35Z" :: TimeStamp) `shouldBe` (TimeStamp 1406849015000000000)
      (read "2014-07-31" :: TimeStamp) `shouldBe` (TimeStamp 1406764800000000000)

    it "reads the Unix epoch date" $ do
      (read "1970-01-01" :: TimeStamp) `shouldBe` (TimeStamp 0)
